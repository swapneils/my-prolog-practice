...
symmetric(tip).
symmetric(bin(tip,_,tip)).
symmetric(bin(bin(A,B,C),_,bin(E,F,G))) :-
    B = F,
    symmetric(bin(A,_,G)),
    symmetric(bin(C,_,E)).
symmetric(_) :- false.
...
insert_tree(X,bin(tip,X,tip),bin(tip,X,tip)).
insert_tree(X,bin(A,B,C),bin(A,B,T)) :-
    X #> B,
    insert_tree(X,C,T).
insert_tree(X,bin(A,B,C),bin(T,B,C)):-
    X #< B,
    insert_tree(X,A,T).
...
%% technopathic travel :)
connect_(manhattan, internet, cafe).
connect_(bronx, internet, cafe).
%% International
connect_(manhattan, britain, ship).
connect_(manhattan, india, plane).
connect_(bronx, britain, ship).
connect_(bronx, india, plane).
connect_(staten_island, britain, ship).
connect_(staten_island, india, plane).
connect_(india, britain, plane).
